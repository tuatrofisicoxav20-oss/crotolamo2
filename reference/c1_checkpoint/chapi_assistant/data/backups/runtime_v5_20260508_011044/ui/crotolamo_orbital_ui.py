...
Fase 4: usa core/crotolamo_runtime.py como tronco común, con seguridad fuerte y datos reales.
...
class CrotolamoOrbitalUI(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("CROTOLAMO :: Orbital Suite v4 Security + Real Panels")
    ...
    def _build_ui(self) -> None:
        ...
        if CORE_IMPORT_ERROR is not None and self.runtime is None:
            ...
            self._log("SISTEMA", "Crotolamo Orbital UI v4 cargado. Seguridad fuerte y datos reales activados, para fingir que los humanos aprendieron arquitectura.")
    ...
    def _build_topbar(self) -> None:
        ...
        tk.Label(title_box, text="クロトラモ  ·  ORBITAL SUITE  ·  v4 SECURITY + REAL PANELS", bg=THEME.bg, fg=THEME.muted, font=("JetBrains Mono", 9)).pack(anchor="w")
    ...
    def execute_current_plan(self) -> None:
        ...
        if not safe or risk == "blocked":
            self._log("SEGURIDAD", "Plan inseguro bloqueado por Seguridad v4.")
            messagebox.showwarning("Crotolamo Seguridad v4", "Plan bloqueado. No lo ejecuto desde la UI.")
            return
    ...
    def _update_connection_panel(self, extra: str = "") -> None:
        ...
        if self.runtime is not None:
            ...
            text = (
                f"runtime=v4  core={core}  skills={skills}  ollama={ollama}\n"
            ...
            text = (
                ...
                "*Modo viejo. Instala runtime v4 si quieres seguridad y datos reales, detalle menor."
            )
        if extra:
            text += "\n\nÚltimo diagnóstico:\n" + extra[-420:]
