"""
Crotolamo Runtime v5.

Fase 5: modos de proyecto + contexto local. La UI, la terminal, la voz y los comandos
...
class CrotolamoRuntime:
    ...
    def _prompt_with_mode_context(self, prompt: str) -> str:
        ...
        if not bool(ctx_cfg.get("inject_mode_context", True)):
            return prompt
        return (
            "CONTEXTO LOCAL DE CROTOLAMO v5:\n"
    ...
    def harden_plan(self, plan: dict[str, Any]) -> dict[str, Any]:
        ...
        mode = self.mode_manager.get_mode()
        if mode:
            explanation += f"\n\nModo activo v5: {mode.get('title')} ({mode.get('key')})."
        if safety.risk == "blocked":
            explanation += "\n\nSeguridad v5: bloqueé el plan porque detecté comandos peligrosos."
        elif safety.risk == "confirm" and commands:
            explanation += "\n\nSeguridad v5: permitido solo con confirmación fuerte. Lee antes de ejecutar, criatura del sudo."
        elif commands:
            explanation += "\n\nSeguridad v5: comandos clasificados como bajo riesgo."
    ...
    def diagnostics_text(self) -> str:
        lines = ["Crotolamo Runtime v5", self.mode_manager.status_text(), "", snapshot_text(self.project_root, settings=self.settings), ""]
