...
Fase 3: usa core/crotolamo_runtime.py como tronco común.
...
# Runtime v3: tronco común oficial. Si falla, caemos al modo viejo para no
...
class CrotolamoOrbitalUI(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("CROTOLAMO :: Orbital Suite v3 Runtime")
    ...
    def _build_ui(self) -> None:
        ...
        if RUNTIME_IMPORT_ERROR is not None:
            self._log("ALERTA", f"Runtime v3 no disponible: {RUNTIME_IMPORT_ERROR}")
        if CORE_IMPORT_ERROR is not None and self.runtime is None:
            ...
            self._log("SISTEMA", "Crotolamo Orbital UI v3 cargado. Ahora el caos pasa por runtime, como proyecto serio y no espagueti con LEDs.")
    ...
    def _build_topbar(self) -> None:
        ...
        tk.Label(title_box, text="クロトラモ  ·  ORBITAL SUITE  ·  v3 RUNTIME CORE", bg=THEME.bg, fg=THEME.muted, font=("JetBrains Mono", 9)).pack(anchor="w")
    ...
    def _build_rightbar(self) -> None:
        ...
        state = OrbitalPanel(right, "ESTADO DEL SISTEMA")
        state.pack(fill="x", pady=(0, 8))
        scan = tk.Canvas(state.body, width=290, height=145, bg=THEME.panel, highlightthickness=0)
        scan.pack(fill="x")
    ...
    def execute_current_plan(self) -> None:
        ...
        if not commands:
            ...
            return
        if not safe:
            self._log("SEGURIDAD", "Plan inseguro bloqueado. Si quieres desastre, mínimo que sea con confirmación explícita.")
            messagebox.showwarning("Crotolamo", "Plan marcado como inseguro. No lo ejecuto desde la UI.")
            return
        if not messagebox.askyesno("Ejecutar comandos", "¿Ejecutar los comandos seguros propuestos?"):
            self._log("SISTEMA", "Ejecución cancelada.")
            return
        self.running = True
        self._set_status("EJECUTANDO", THEME.green)
        threading.Thread(target=self._worker_execute, args=(commands,), daemon=True).start()

    def _worker_execute(self, commands: list[str]) -> None:
        if self.runtime is not None:
            for event in self.runtime.execute_commands(commands):
    ...
    def _update_connection_panel(self, extra: str = "") -> None:
        ...
        if self.runtime is not None:
            ...
            text = (
                f"runtime=v3  core={core}  skills={skills}  ollama={ollama}\n"
            ...
            text = (
                ...
                "*Modo viejo. Instala runtime v3 si quieres que deje de dar pena estructural."
            )
        if extra:
            text += "\n\nÚltimo diagnóstico:\n" + extra[-420:]
