...
Fase 6: usa core/crotolamo_runtime.py como tronco común, con modos de proyecto y contexto local.
...
# Runtime v6: tronco común oficial. Si falla, caemos al modo viejo para no
...
class CrotolamoOrbitalUI(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("CROTOLAMO :: Orbital Suite v6 Project Modes")
    ...
    def _build_ui(self) -> None:
        ...
        if RUNTIME_IMPORT_ERROR is not None:
            self._log("ALERTA", f"Runtime v6 no disponible: {RUNTIME_IMPORT_ERROR}")
        if CORE_IMPORT_ERROR is not None and self.runtime is None:
            ...
            self._log("SISTEMA", "Crotolamo Orbital UI v6 cargado. Modos de proyecto y contexto local activados, porque responder todo en genérico era demasiado siglo pasado.")
    ...
    def _build_topbar(self) -> None:
        ...
        tk.Label(title_box, text="クロトラモ  ·  ORBITAL SUITE  ·  v6 PROJECT MODES", bg=THEME.bg, fg=THEME.muted, font=("JetBrains Mono", 9)).pack(anchor="w")
    ...
    def _build_rightbar(self) -> None:
        ...
        mode_panel = OrbitalPanel(right, "MODO ACTIVO v6", accent=THEME.magenta)
    ...
    def execute_current_plan(self) -> None:
        ...
        if not safe or risk == "blocked":
            self._log("SEGURIDAD", "Plan inseguro bloqueado por Seguridad v6.")
            messagebox.showwarning("Crotolamo Seguridad v6", "Plan bloqueado. No lo ejecuto desde la UI.")
            return
    ...
    def _update_connection_panel(self, extra: str = "") -> None:
        ...
        if self.runtime is not None:
            ...
            text = (
                f"runtime=v6  modo={mode}  core={core}  skills={skills}  ollama={ollama}\n"
            ...
            text = (
                ...
                "*Modo viejo. Instala runtime v6 si quieres seguridad y datos reales, detalle menor."
            )
        if extra:
            text += "\n\nÚltimo diagnóstico:\n" + extra[-420:]
